...
pub async fn start_proxy_service(
    ...
    let (axum_server, server_handle) =
        match crate::proxy::AxumServer::start(
            ...
            config.zai.clone(),
            config.copilot.clone(),  // ⚠️ Copilot provider (非官方 API)
            monitor.clone(),
...
/// 开始 Copilot GitHub Device Code Flow
#[tauri::command]
pub async fn start_copilot_device_flow() -> Result<CopilotDeviceCode, String> {
    let response = crate::proxy::providers::copilot::start_device_flow().await?;
...
/// 轮询等待 Copilot OAuth 授权
#[tauri::command]
pub async fn poll_copilot_oauth_token(device_code: String, interval: u64) -> Result<String, String> {
    crate::proxy::providers::copilot::poll_for_token(device_code, interval).await
}

/// 使用 OAuth Token 获取 Copilot Token
#[tauri::command]
pub async fn get_copilot_token_from_oauth(oauth_token: String) -> Result<String, String> {
    let (copilot_token, _expires_at) =
        crate::proxy::providers::copilot::get_copilot_token_with_oauth(&oauth_token).await?;
    Ok(copilot_token)
}
