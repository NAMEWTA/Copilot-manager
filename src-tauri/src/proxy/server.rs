...
pub struct AppState {
    ...
    pub copilot: Arc<RwLock<crate::proxy::CopilotConfig>>,
...
pub struct AxumServer {
    ...
    /// ⚠️ Copilot provider state (非官方 API，使用风险自负)
    copilot_state: Arc<RwLock<crate::proxy::CopilotConfig>>,
...
impl AxumServer {
    ...
    /// 更新 Copilot 配置
    /// ⚠️ 非官方 API，使用风险自负
    pub async fn update_copilot(&self, config: &crate::proxy::config::ProxyConfig) {
        let mut copilot = self.copilot_state.write().await;
        // 保留现有的 token 缓存
        let existing_token = copilot.copilot_token.clone();
        let existing_expires = copilot.token_expires_at;

        *copilot = config.copilot.clone();

        // 如果新配置没有 token，保留旧的缓存
        if copilot.copilot_token.is_none() && existing_token.is_some() {
            copilot.copilot_token = existing_token;
            copilot.token_expires_at = existing_expires;
        }

        tracing::info!("⚠️ Copilot 配置已热更新（非官方 API，使用风险自负）");
    }
    ...
    pub async fn start(
        ...
        copilot_config: crate::proxy::CopilotConfig,
        ...
        let custom_mapping_state = Arc::new(tokio::sync::RwLock::new(custom_mapping));
	        ...
	        let copilot_state = Arc::new(RwLock::new(copilot_config));
