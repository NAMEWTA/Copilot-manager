...
pub async fn handle_chat_completions(
    ...
    // ===== Copilot Provider 检查 =====
    // ⚠️ 非官方 API，使用风险自负
    let copilot = state.copilot.read().await.clone();
    debug!("Copilot config: enabled={}, dispatch_mode={:?}", copilot.enabled, copilot.dispatch_mode);
    let copilot_enabled = copilot.enabled
        && !matches!(copilot.dispatch_mode, crate::proxy::CopilotDispatchMode::Off);

    // 判断是否使用 Copilot
    let use_copilot = if !copilot_enabled {
        false
    } else {
        let google_pool_size = state.token_manager.len();
        match copilot.dispatch_mode {
            crate::proxy::CopilotDispatchMode::Exclusive => {
                debug!("Copilot dispatch mode: Exclusive - using Copilot for all requests");
                true
            }
            crate::proxy::CopilotDispatchMode::Fallback => {
                // 仅在 Google 池无可用账号时使用 Copilot
                if google_pool_size == 0 {
                    info!("Google pool empty, using Copilot as fallback");
                    true
                } else {
                    false
                }
            }
            crate::proxy::CopilotDispatchMode::Pooled => {
                // Copilot 作为共享池中的一个槽位
                let total = google_pool_size.saturating_add(1).max(1);
                let slot = state.provider_rr.fetch_add(1, std::sync::atomic::Ordering::Relaxed) % total;
                // Copilot 占最后一个槽位
                slot >= google_pool_size
            }
            crate::proxy::CopilotDispatchMode::Off => false,
        }
    };

    // 如果选择使用 Copilot，直接转发
    if use_copilot {
        info!("⚠️ Routing request to Copilot (non-official API, use at your own risk)");
        // 将 OpenAIRequest 转回 Value 用于转发
        let request_body = match serde_json::to_value(&openai_req) {
            Ok(v) => v,
            Err(e) => return Err((StatusCode::INTERNAL_SERVER_ERROR, format!("Serialization error: {}", e))),
        };

        // 调用 Copilot provider
        let response = crate::proxy::providers::copilot::forward_to_copilot(
            &state,
            axum::http::Method::POST,
            "/chat/completions",
            &axum::http::HeaderMap::new(),  // TODO: 传入实际 headers
            request_body,
        ).await;

        return Ok(response);
    }
    // ===== Copilot Provider 检查结束 =====
