...
pub async fn handle_messages(
    ...
    let use_copilot = if !copilot_enabled || use_zai {
        // Copilot 未启用，或 z.ai 已处理
        false
    } else {
        match copilot.dispatch_mode {
            crate::proxy::CopilotDispatchMode::Off => false,
            crate::proxy::CopilotDispatchMode::Exclusive => {
                tracing::info!("[{}] Copilot dispatch mode: Exclusive - using Copilot for all requests", trace_id);
                true
            }
            crate::proxy::CopilotDispatchMode::Fallback => {
                if google_accounts == 0 {
                    tracing::info!("[{}] No Google accounts available, using Copilot as fallback", trace_id);
                    true
                } else {
                    let has_available = state.token_manager.has_available_account("claude", &normalized_model).await;
                    if !has_available {
                        tracing::info!(
                            "[{}] All Google accounts unavailable, using Copilot fallback",
                            trace_id
                        );
                    }
                    !has_available
            ...
            crate::proxy::CopilotDispatchMode::Pooled => {
                // Copilot 作为共享池中的一个槽位（在 z.ai 之后）
                let total = google_accounts.saturating_add(1).max(1);
                let slot = state.provider_rr.fetch_add(1, Ordering::Relaxed) % total;
                slot >= google_accounts
            }
        }
    };
    ...
    // 如果选择使用 Copilot，直接转发
    if use_copilot {
        tracing::info!("⚠️ [{}] Routing request to Copilot (non-official API, use at your own risk)", trace_id);

        // 将 ClaudeRequest 转换为 Anthropic 格式的 Value
        ...
        // 调用 Copilot provider（Copilot 支持 Anthropic 协议）
        return crate::proxy::providers::copilot::forward_to_copilot(
            &state,
