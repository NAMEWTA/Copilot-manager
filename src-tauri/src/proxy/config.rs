...
/// Copilot 分发模式 (与 ZaiDispatchMode 类似)
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "snake_case")]
pub enum CopilotDispatchMode {
    /// 从不使用 Copilot
    Off,
    /// 所有 Anthropic/OpenAI 协议请求都使用 Copilot
    Exclusive,
    /// 将 Copilot 视为共享池中的一个额外槽位
    Pooled,
    /// 仅在 Google 池不可用时使用 Copilot
    Fallback,
}

impl Default for CopilotDispatchMode {
    fn default() -> Self {
        Self::Off
    }
}
...
/// Copilot 配置 (类似 ZaiConfig)
/// 用于管理 GitHub Copilot 作为上游提供商
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CopilotConfig {
    /// 是否启用 Copilot provider
    #[serde(default)]
    pub enabled: bool,

    /// GitHub Token (Personal Access Token 或 OAuth Token)
    /// 用于获取 Copilot 内部 token
    #[serde(default)]
    pub github_token: String,

    /// 分发模式
    #[serde(default)]
    pub dispatch_mode: CopilotDispatchMode,

    /// Copilot 内部 token (缓存，不序列化到配置文件)
    #[serde(skip)]
    pub copilot_token: Option<String>,

    /// Token 过期时间戳 (Unix timestamp)
    #[serde(skip)]
...
impl Default for CopilotConfig {
    fn default() -> Self {
        Self {
            ...
            dispatch_mode: CopilotDispatchMode::Off,
            copilot_token: None,
...
fn default_copilot_model() -> String {
    "claude-opus-4.5".to_string()  // 默认使用最强的 Claude 模型
}
...
pub struct ProxyConfig {
    ...
    /// Copilot provider configuration (GitHub Copilot as upstream).
    /// ⚠️ 非官方 API，使用风险自负
    #[serde(default)]
